...
// Base62 reference table for encoding
...
module.exports = async (app, database) => {
    ...
    app.get('/shortify/:hashValue', (req, res) => {
        const shortUrl = 'http://localhost:3000/shortify/' + req.params.hashValue;
        ...
        connection.query('SELECT * FROM Container WHERE shortUrl=?', [shortUrl], (err, rows) => {
            ...
            // Uncomment below if you want to redirect instead of returning JSON
            // res.redirect(ogUrl);
        });
    });
    ...
    app.post('/shortify/postUrl', (req, res) => {
        ...
        // check if a short url corresponding to this URL exists already
        connection.query(`SELECT * FROM Container WHERE longUrl=?`, [ogUrl], (err, rows) => {
            ...
            if (rows.length === 0) {
                ...
                var shortUrl = "http://localhost:3000/shortify/" + hashValue;
